...
@section('content')
    <div class="flex justify-between items-center">
        <h3 class="text-gray-700 text-3xl font-medium">Tambah prodi</h3>
                ...
                <form action="{{ route('prodi.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" name="id_prodi" id="id_prodi" value="{{ old('id_prodi') }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('id_prodi') border-red-500 @enderror" required>
                            @error('id_prodi')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
